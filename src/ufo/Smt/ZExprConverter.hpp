...
{
  template <typename C>
  static z3::ast marshal(Expr e, z3::context &ctx, C &cache, expr_ast_map &seen)
  {
...
struct FailUnmarshal
{
  template <typename C>
  static Expr
  unmarshal(const z3::ast &a, ExprFactory &efac, C &cache, ast_expr_map &seen)
  {
...
struct BasicExprMarshal
{
  ...
  static z3::ast marshal(Expr e, z3::context &ctx, C &cache, expr_ast_map &seen)
  {
    ...
    else if(bind::isFdecl(e))
    {
      ...
      z3::sort range(
        ...
        reinterpret_cast<Z3_sort>(
          static_cast<Z3_ast>(marshal(bind::rangeTy(e), ctx, cache, seen))));
    ...
    else if(bind::isFapp(e))
    {
      z3::func_decl zfdecl(
        ...
        reinterpret_cast<Z3_func_decl>(
          static_cast<Z3_ast>(marshal(bind::fname(e), ctx, cache, seen))));
    ...
    else if(isOpX<FORALL>(e) || isOpX<EXISTS>(e))
    {
      ...
      z3::ast ast(
        marshal(e->last(), ctx, cache, seen)); //z3.toAst (e->last()));
      ...
      if(isOpX<FORALL>(e))
        res = Z3_mk_forall_const(ctx, 0, bound.size(), &bound[0], 0, NULL, ast);
      else
        res = Z3_mk_exists_const(ctx, 0, bound.size(), &bound[0], 0, NULL, ast);
    }
    ...
    else if(arity == 2)
    {
      ...
      else if(isOpX<BSEXT>(e) || isOpX<BZEXT>(e))
      {
        ...
        if(isOpX<BSEXT>(e))
          res =
            z3::ast(ctx, Z3_mk_sign_ext(ctx, bv::width(e->arg(1)) - t1_sz, t1));
        else if(isOpX<BZEXT>(e))
          res =
            z3::ast(ctx, Z3_mk_zero_ext(ctx, bv::width(e->arg(1)) - t1_sz, t1));
        else
          assert(0);
      }
    ...
    else if(isOpX<BEXTRACT>(e))
    {
      ...
      isOpX<AND>(e) || isOpX<OR>(e) || isOpX<ITE>(e) || isOpX<XOR>(e) ||
      isOpX<PLUS>(e) || isOpX<MINUS>(e) || isOpX<MULT>(e) || isOpX<STORE>(e) ||
      isOpX<ARRAY_MAP>(e))
    {
...
struct BasicExprUnmarshal
{
  template <typename C>
  static Expr
  unmarshal(const z3::ast &z, ExprFactory &efac, C &cache, ast_expr_map &seen)
  {
    ...
    else if(kind == Z3_SORT_AST)
    {
      ...
      case Z3_ARRAY_SORT:
        domain = unmarshal(
          z3::ast(
            ctx, Z3_sort_to_ast(ctx, Z3_get_array_sort_domain(ctx, sort))),
          efac,
          cache,
          seen);
        range = unmarshal(
    ...
    else if(kind == Z3_FUNC_DECL_AST)
    {
      ...
      for(unsigned p = 0; p < Z3_get_domain_size(ctx, fdecl); ++p)
      {
        ...
        type.push_back(unmarshal(
          z3::ast(ctx, Z3_sort_to_ast(ctx, sort)), efac, cache, seen));
      }

      type.push_back(unmarshal(
        z3::ast(ctx, Z3_sort_to_ast(ctx, Z3_get_range(ctx, fdecl))),
        efac,
        cache,
        seen));
    ...
    else if(kind == Z3_QUANTIFIER_AST)
    {
      ...
      args.push_back(unmarshal(
        z3::ast(ctx, Z3_get_quantifier_body(ctx, z)), efac, cache, seen));
    ...
    if(dkind == Z3_OP_NOT)
    {
      ...
      return mk<NEG>(unmarshal(
        z3::ast(ctx, Z3_get_app_arg(ctx, app, 0)), efac, cache, seen));
    }
    ...
    if(dkind == Z3_OP_BNOT)
      return mk<BNOT>(unmarshal(
        z3::ast(ctx, Z3_get_app_arg(ctx, app, 0)), efac, cache, seen));
    if(dkind == Z3_OP_BNEG)
      return mk<BNEG>(unmarshal(
        z3::ast(ctx, Z3_get_app_arg(ctx, app, 0)), efac, cache, seen));
    if(dkind == Z3_OP_BREDAND)
      return mk<BREDAND>(unmarshal(
        z3::ast(ctx, Z3_get_app_arg(ctx, app, 0)), efac, cache, seen));
    if(dkind == Z3_OP_BREDOR)
      return mk<BREDOR>(unmarshal(
        z3::ast(ctx, Z3_get_app_arg(ctx, app, 0)), efac, cache, seen));
