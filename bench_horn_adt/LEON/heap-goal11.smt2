...
(declare-fun heapsorta (Heap Lst) Bool)
(assert (heapsorta hleaf nil))
(assert (forall ((k Int) (v Int) (l Heap) (r Heap) (h Heap) (m Lst))
	(=> (and (merge l r h) (heapsorta h m)) (heapsorta (heap k v l r) (cons v m)))))

(declare-fun heapsort (Lst Lst) Bool)
(assert (forall ((l Lst) (h Heap) (m Lst)) 
	(=> (and (hinsert-all l hleaf h) (heapsorta h m)) (heapsort l m))))

; conjecture
(assert (forall ((l Lst) (k Lst))
	(=> (and (heapsort l k) (not (sorted k))) false)))
