#ifndef TGRNDLEARNERV3__HPP__
#define TGRNDLEARNERV3__HPP__

#include "RndLearnerV2.hpp"
#include "TGLBExpl.hpp"
#include <ctime>
      ...
      for (auto mbp : cur_mbps)
      {
        ...
        if (dAddDat)
        {
              ...
              candsToDat.insert(inv);
          {
            ...
            for (auto & q : qvits[invNum])
              candToDat = replaceAll(candToDat, q->qv, q->iter);
...
//        }
    ...
    Expr getDataCandidates(map<Expr, ExprSet>&, Expr = NULL,
                           Expr = NULL, Expr = NULL)
    {
      if (printLog)
        outs() << "Skipping TG data learning: the data learner is not available in this tree\n";
      return NULL;
    }
    ...
    void initializeAux(Expr rel, TGBndExpl& bnd, int cycleNum, Expr pref)
    {
  ...
  inline void testgen(string smt, set<int>& nums, unsigned maxAttempts,
                      unsigned to, unsigned all_to,
                      bool freqs, bool aggp, bool enableDataLearning, int doElim,
                      bool doDisj, int doProp, bool dAllMbp, bool dAddProp, bool dAddDat,
                      bool dStrenMbp, int mbpEqs, bool toSkip, int invMode, int lookahead,
                      bool lb, bool lmax, bool prio, bool prune, int debug)
  {
    ...
    auto pref = smt.substr(smt.find_last_of('/') + 1,
        smt.find_last_of('.') - smt.find_last_of('/') - 1);

    if (invMode == 0)
    {
      ruleManager.parse(smt, doElim, nums);
      ruleManager.reParse(lb, lmax);
      outs () << "printing " << ruleManager.chcs.size() << "\n";
      outs().flush();
      for (int i = 0; i < ruleManager.chcs.size(); i++)
        if (!ruleManager.chcs[i].isFact && !ruleManager.chcs[i].isQuery)
          ruleManager.serialize(pref, i);
      exit (1);
    }
    else
    {
      ...
      {
        TGBndExpl bnd(ruleManager, false);
    ...
    if (nums.size() > 0)
    {
      ...
      int sz = ruleManager.acyclic.size();
      if (sz == 0) exit(0);

      auto spent = std::difftime(std::time(nullptr), tm);

      int allt = (all_to-spent)/sz;
      if (allt > 30) allt = allt - 10;

      outs () << "number of globals: " << sz << "\n";
      outs () << "time chunk: " << allt << "\n";
      ...
      if (allt > 30)
      {
        ...
        int tn = 0;
        for (auto c : ruleManager.acyclic)
        {
          vector<vector<int>> init = {c};
          TGLBExpl bnd1(ruleManager, lookahead, tn, prio, false, to, allt, debug);
          outs () << "Restart with global path\n";
          bnd1.print(c);

          bnd1.initKeys(nums, lb);
          bnd1.setInvs(invs);
          bnd1.exploreTracesMaxLb(init);
          tn = bnd1.getTestNum();
        }
      }
      else
      {
        outs () << "RUNNING ORIG FOR LONG\n";

        // original version
        TGLBExpl bnd1(ruleManager, lookahead, 0, prio, /*prune*/ true, to, all_to, debug);
        bnd1.initKeys(nums, lb);
        bnd1.setInvs(invs);
        if (lb && lmax)
          bnd1.exploreTracesMaxLb(ruleManager.acyclic);
        else if (lb)
