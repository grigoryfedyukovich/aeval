  ...
  struct BasicExprMarshal
  {
                           ...
                           C &cache, expr_ast_map &seen)
    {
      ...
      else if (bind::isFdecl(e))
      {
        ...
        z3::sort range(ctx,
                       reinterpret_cast<Z3_sort>(static_cast<Z3_ast>(marshal(bind::rangeTy(e), ctx, cache, seen))));
      ...
      else if (bind::isFapp(e))
      {
        ...
        z3::func_decl zfdecl(ctx,
                             reinterpret_cast<Z3_func_decl>(static_cast<Z3_ast>(marshal(bind::fname(e), ctx, cache, seen))));
                          ...
                          ast_expr_map &seen)
    {
      ...
      else if (kind == Z3_QUANTIFIER_AST)
      {
        ...
        args.push_back(unmarshal(z3::ast(ctx, Z3_get_quantifier_body(ctx, z)),
                                 efac, cache, seen));
        return Z3_is_quantifier_forall(ctx, z) ? mknary<FORALL>(args) : mknary<EXISTS>(args);
      }
      ...
      {
        z3::ast zdecl(ctx, Z3_func_decl_to_ast(ctx,
      ...
      for (size_t i = 0; i < (size_t)Z3_get_app_num_args(ctx, app); i++)
        args.push_back(unmarshal(z3::ast(ctx, Z3_get_app_arg(ctx, app, i)), efac, cache, seen));
      ...
      case Z3_OP_CONST_ARRAY:
      {
        assert(args.size() == 1);
        Z3_sort sort = Z3_get_sort(ctx, z);
        Expr domain = unmarshal(z3::ast(ctx, Z3_sort_to_ast(ctx,
