...
namespace z3
{
  ...
  {
    Z3_ast_map m_map;
    void init(Z3_ast_map v)
    {
      Z3_ast_map_inc_ref(ctx(), v);
      m_map = v;
    }
  ...
  public:
    ...
    ast find(ast const &k)
    {
      Z3_ast res = Z3_ast_map_find(ctx(), m_map, k);
      check_error();
      return ast(ctx(), res);
    };
    ast_vector get_keys()
    {
      Z3_ast_vector res = Z3_ast_map_keys(ctx(), m_map);
      check_error();
      return ast_vector(ctx(), res);
    };

    friend std::ostream &operator<<(std::ostream &out, ast_map const &v)
    {
      out << Z3_ast_map_to_string(v.ctx(), v);
      return out;
    }
  };
}
...
namespace ufo
{
  ...
  public:
    ...
    friend Expr z3_qe_model_project_skolem<this_type, this_model_type>(this_type &z3, this_model_type &model, Expr v, Expr body, ExprMap &map);
  ...
  public:
    ...
    ZModel(Z &z, const z3::model &m) : z3(z), ctx(z.get_ctx()), model(m), efac(z.get_efac())
    {
  ...
  public:
    ...
    ZSolver(Z &z, const char *logic) : z3(z), ctx(z.get_ctx()), solver(z.get_ctx(), logic), efac(z.get_efac())
    {
      solver.push();
    }
  ...
  public:
    ...
    ZFixedPoint(Z &z) : z3(z), ctx(z.get_ctx()), fp(z.get_ctx()), efac(z.get_efac()) {}
