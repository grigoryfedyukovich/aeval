...
; lemmas for len after remove
(assert (forall ((i Int) (xs Lst))
    (=> (< i (len xs))
        (= (len (remove i xs)) (- (len xs) 1)))))

; lemmas for get after remove
(assert (forall ((i Int) (j Int) (xs Lst))
    (=> (and (< i (- (len xs) 1)) (< j (len xs)))
        (= (get i (remove j xs))
           (ite (< i j) (get i xs)
                        (get (+ i 1) xs))))))
